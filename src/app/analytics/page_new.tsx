...
export default function AnalyticsPage() {
  ...
  // 로컬 포트폴리오 데이터를 기반으로 분석 데이터 계산
  const calculateAnalytics = useCallback(async (): Promise<PortfolioAnalytics | null> => {
    ...
    // 주식 코드 목록 추출
    const stockCodes = Array.from(new Set(portfolioData.holdings.map(h => h.stockCode)));
    
    // 현재 주가 정보 가져오기
    const currentPrices: { [key: string]: StockData } = {};
    try {
      const response = await fetch(`/api/stock-prices?codes=${stockCodes.join(',')}`);
      if (response.ok) {
    ...
    for (const holding of portfolioData.holdings) {
      ...
      performers.push({
        ...
        name: currentPrices[holding.stockCode]?.name || holding.stockCode,
        returnPercent,
        value: currentValue
      });
    }
  ...
  const fetchStockPrices = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const formattedPrices: { [key: string]: StockPrice } = {};
        
        Object.entries(data).forEach(([code, stockData]: [string, any]) => {
        ...
        setStockPrices(formattedPrices);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              <h1 className="text-xl font-semibold text-gray-900 dark:text-white">포트폴리오 분석</h1>
            </div>
